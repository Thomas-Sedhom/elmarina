...
const entryInput = z.object({
  brokerAccountId: z.coerce.number().int().positive(),
...
const parseInput = (body: unknown) => {
  ...
  return { ...input, businessDate: new Date(input.businessDate), notes: input.notes ?? null };
};
...
export const createEntry = asyncHandler(async (req: Request, res: Response) => sendData(res, await ledgerService.create({ ...parseInput(req.body), createdBy: req.user!.id, updatedBy: req.user!.id }), 201));
export const updateEntry = asyncHandler(async (req: Request, res: Response) => {
  await ledgerService.update(idSchema.parse(req.params.id), { ...parseInput(req.body), createdBy: req.user!.id, updatedBy: req.user!.id });
