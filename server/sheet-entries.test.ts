...
import { canUserViewBrokerAccount, appRouter } from "./routers";
...
const adminContext = { user: user(1, "admin"), req: { protocol: "https", headers: {} } as any, res: { cookie: vi.fn(), clearCookie: vi.fn() } as any };
...
describe("sheet entry backend procedures", () => {
  it("guards and maps create through the admin procedure", async () => {
    ...
    await appRouter.createCaller(adminContext).sheetEntries.create({ brokerAccountId: 4, businessDate: "2026-08-14T00:00:00.000Z", weight: "2.500", description: "خاتم", cash: "900.00", notes: null, type: "work" });
    expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({ createdBy: 1, updatedBy: 1, brokerAccountId: 4, type: "work" }));
    createSpy.mockRestore();
  });
  it("routes update and delete through protected admin mutations", async () => {
    ...
    updateSpy.mockRestore(); deleteSpy.mockRestore();
  });
  it("rejects a broker attempting to access a different account through the procedure", async () => {
    ...
    await expect(appRouter.createCaller({ ...adminContext, user: user(7, "broker") }).brokers.get({ id: 4 })).rejects.toThrowError(/only view your own account/);
    accountSpy.mockRestore();
  });
});

describe.skipIf(!process.env.MONGODB_URI)("real MongoDB transactional ledger flows", () => {
