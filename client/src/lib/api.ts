...
export const api = {
  ...
  useUtils: () => {
    ...
    return { auth: { me: { invalidate: () => queryClient.invalidateQueries({ queryKey: ["auth", "me"] }) } }, brokers: { list: { invalidate: () => queryClient.invalidateQueries({ queryKey: ["brokers", "list"] }) }, get: { invalidate: (input: { id: string }) => queryClient.invalidateQueries({ queryKey: ["brokers", "get", input.id] }) } }, entries: { list: { invalidate: (input: { brokerAccountId: string }) => queryClient.invalidateQueries({ queryKey: ["entries", "list", input.brokerAccountId] }) } } };
  },
};
